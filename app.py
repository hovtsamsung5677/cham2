...
# ---------- ВКЛЮЧАЕМ OPTIMIZATION ДЛЯ TENSOR CORES ----------
...
def recolor_image_preserve_texture(image_pil, mask_pil, target_r, target_g, target_b):
    """
    Recolor image preserving texture using HSV blend on selected area.
    This replaces the hue while preserving original value (luminance/shadows).
    """
    ...
    # Convert target to HSV to get hue and saturation
    ...
    for y in range(h):
        for x in range(w):
            mask_idx = y * w + x
            if mask_np[mask_idx] > 0.5:
                r, g, b = image_np[y, x]
                
                # Get original value (luminance) - preserves texture/shadows
                value = max(r, g, b)
                
                # Apply target hue with preserved value
                c = value * t_sat
...
async def ai_recolor(
    ...
    try:
        ...
        # 5. Перекрашивание с сохранением текстуры (HSV blend)
